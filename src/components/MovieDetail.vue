    ...
    <v-container>
        <v-row>
            ...
            <v-col cols="12" sm="8">
                ...
                <div class="mt-5">
                    <h2 class="mt-5 grey--text text--darken-3">Elenco em destaque</h2>
                ...
                >
                  <template v-slot:activator="{on, attrs}">
                    <v-btn tile style="background: rgb(3, 37, 65); color: white;" v-bind="attrs" v-on="on" @click.prevent="openYouTubeModel">
                      <v-icon left>mdi-play</v-icon>Trailer
                    </v-btn>
                  </template>
                  <v-card>
                    <v-card-title>
                    ...
                    <v-card-actions>
                      <v-spacer></v-spacer>
                      <v-btn style="background: rgb(3, 37, 65); color: white;" text @click="dialog = flase">Fechar</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-dialog>
                <v-btn tile style="background: rgb(3, 37, 65); color: white;" class="ml-2">
                    <v-icon left>mdi-heart</v-icon>Favorite
                </v-btn>
            </v-col>
          </v-row> 
        <v-divider class="mt-2"></v-divider>
         <CastMovie :casts="movie.credits.cast"/>
         <v-divider class="mt-2"></v-divider>
         <ImagesMovie :images="movie.images.backdrops" />
    </v-container>
  </div>
</template>
<script>
  import CastMovie from "../components/CastMovie.vue"
  import ImagesMovie from "../components/ImagesMovie.vue"
  export default {
  components: {
    CastMovie,
    ImagesMovie,
